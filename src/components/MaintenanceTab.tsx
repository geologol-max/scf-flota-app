...
import { Wrench, Calendar, Plus, Coins, Search, FileText, ClipboardList, Printer, Edit3, Trash2 } from 'lucide-react';
import { exportMaintenanceOrderToPDF, downloadMaintenanceOrderAsPDF } from '../utils/exporters';
...
export default function MaintenanceTab({ logs, vehicles, onUpdateLogs, permissions, currentUserRole }: MaintenanceTabProps) {
  ...
  const handleEdit = (log: MaintenanceLog) => {
    setEditingLog(log);
    setPlate(log.ppu);
    setServiceType(log.tipo_servicio);
    setDate(log.fecha);
    setOdometer(log.kilometraje);
    setCost(log.costo);
    setDescription(log.descripcion);
    setWorkshop(log.taller);
  ...
  const handleDelete = (id: string) => {
    ...
    if (confirm('¿Está seguro de eliminar este registro de mantención?')) {
                    ...
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            const matchVehicle = vehicles.find(v => v.ppu === log.ppu);
                            downloadMaintenanceOrderAsPDF(log, matchVehicle);
                          }}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-[11px] font-bold transition-all cursor-pointer border border-emerald-100 shadow-xs"
                          title="Descargar Orden de Trabajo en PDF"
                          id={`btn-download-ot-${log.id}`}
                        >
                          <FileText className="w-3.5 h-3.5 text-emerald-600" />
                          <span>Descargar OT PDF</span>
                        </button>
